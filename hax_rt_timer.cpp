...
hax_rt_timer::hax_rt_timer(uint32_t timer_period, uint64_t timer_offset, const char * name) {
...
void hax_rt_timer::wait_next_activation() {
  ...
  int err = clock_nanosleep(CLOCK_REALTIME, TIMER_ABSTIME, &m_nanotime, NULL);
  assert(err == 0);
